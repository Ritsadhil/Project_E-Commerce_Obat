...
<body class="h-screen flex items-center justify-center font-sans bg-cover bg-center bg-no-repeat bg-gray-100"
    ...
    <div class="gradient-card rounded-3xl shadow-2xl w-[90%] h-[92vh] p-6 flex flex-col overflow-hidden relative">
        <form action="{{ route('obat.destroy', $medicine->id) }}" method="POST" class="absolute top-6 right-6 z-10">
            @csrf
            @method('DELETE')
            <button type="submit" 
                onclick="return confirm('⚠️ Yakin ingin menghapus obat ini? Data tidak bisa kembali.')" 
                class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-xl shadow-md border border-red-400 flex items-center gap-2 text-sm transition-all">
